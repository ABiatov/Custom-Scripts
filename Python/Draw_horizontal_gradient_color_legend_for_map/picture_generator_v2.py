...
for i, label in enumerate(label_list):
    ...
    label_x = 1 if i == 0 else (width - label_size[0] - 11 if i == len(label_list) - 1 else int(i * (width - label_size[0]) / (len(label_list) - 1)))
    label_y = height - label_size[1] - 15
    label_positions.append((label_text, label_x, label_y))

# Draw the labels
for label_text, label_x, label_y in label_positions:
